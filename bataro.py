...
class Models:
	...
	def _stage(self):
		...
		self.grids = jnp.array([np.r_[g, np.zeros((max_grid_len - len(g), 3))] for g in _grids])
...
def optimise_stimulus(target_neurons, models, learning_rate=3e1, iters=100, n_inits=5, init_spread=5, I_max=70.):
	...
	x = jnp.c_[
		models.neuron_locations[tuple(target_neurons), :] + np.random.normal(0, init_spread, [n_inits, len(target_neurons), 2]), 
	...
	x_path = jnp.zeros((n_inits, iters+1, len(target_neurons), 3))
